...
| `remoteRulesEnabled` | boolean | `true` | On by default (#888): fetches Ed25519-signed rule updates at most once per 7 days (`credentials: "omit"`, no cookies, no identifiers). A fresh install makes this one outbound GET to rules.muga.app; disable it and both short-link switches (`resolveShortenersOnClick`, `resolveShortenersOnHover`) in Settings for zero network activity. Supersedes REQ-OPT-1. |
...
| `crossSiteFrequencyEnabled` | boolean | `true` | (#446, B16) Local-only Cross-Site Frequency Tracker: maintains a `(paramName, sha256(value))` map keyed per first-party domain so Settings > Activity can flag params that appear on 3+ domains AND have 3+ distinct values. LRU-capped at 1000 unique params. NEVER transmitted. Toggle off to make observations a no-op and hide the freq subgroup. |
| `attributionLedgerEnabled` | boolean | `true` | (#460, A2) Persist the Attribution Ledger ring buffer to `chrome.storage.local["attributionLedger"]` so the "Recent activity" list in Settings > Activity survives service-worker restarts. Toggle off to gate the SW writer (no new local-storage writes). |
...
| `userCustomRules` | string[] | `[]` | (#536) Per-user custom strip rules promoted from the "Strip everywhere" button on flagged Suspicious-params rows in Settings > Activity. Each entry is a bare param name (matched case-insensitively by the cleaner) stripped on EVERY host. Lives in sync so promotions follow the user across devices. Affiliate-preservation always wins (the affiliateParamSet skip in `stripTrackingParams` runs first), so a user can never accidentally strip their own creator referral tag. |
| `resolveShortenersOnClick` | boolean | `true` (every browser) | (browsewrap Phase 2, follow-up to ADR-0004 phases 2-5 #699/#701) Click-time shortener resolution: when ON, `src/content/cleaner.js` resolves a clicked generic shortener (bit.ly, tinyurl.com, etc.) in-browser via a `RESOLVE_SHORTENER` (`source: "click"`) round trip to the service worker, which fetches via `fetch(redirect:"follow")` and reads `response.url`, so the whole redirect chain is contacted. No MUGA server is involved. The shortener hosts are declared as optional host permissions, but `<all_urls>` already covers them; this pref is the gate. Default `true` on every browser — the user was already navigating to this link, so resolving it is low-risk. Split from the retired `followShortenersEnabled` (which gated both click and hover together); a one-time startup migration (`migrateFollowShortenersSplit`) maps an explicitly-stored prior value onto both new prefs, then deletes the old key. |
| `resolveShortenersOnHover` | boolean | `false` (every browser, opt-in) | (browsewrap Phase 2) Hover/proactive shortener resolution: when ON, `src/content/hover-preview.js` resolves a hovered generic shortener the same way, via `RESOLVE_SHORTENER` with `source: "hover"`. Default `false` on every browser — pinging the shortener host for a link the user only looked at (never clicked) leaks "the user saw this link" to a third party they never chose to visit, a privacy cost click-time resolution doesn't have, so this stays opt-in. Requires the same shortener host permissions as `resolveShortenersOnClick`. The service worker's `RESOLVE_SHORTENER` handler is source-gated defense-in-depth: it re-checks the pref matching `message.source` itself rather than trusting the caller. |
| `hoverPreviewEnabled` | boolean | `true` | (#1028) Desktop-only: `src/content/hover-preview.js` shows a small text-only tooltip with the real cleaned destination when the user hovers AND holds still over a link for `HOVER_PREVIEW_DELAY_MS` (2500ms, fixed in code — see below). Local-only (no network, no new permissions) when local unwrap/clean changes the link's host (wrappers / redirect networks). For a generic shortener link whose host does NOT change locally, it ALSO resolves the destination over the network via the existing `RESOLVE_SHORTENER` round trip — but only when `resolveShortenersOnHover` is ON, reusing that opt-in rather than requesting a new permission. Default ON. |
...
| `attributionLedger` | object | `{ events: [], capacity: 10 }` | (#460, A2) Rolling ring buffer of the last cleaner-pipeline events feeding the "Recent activity" list in Settings > Activity. Shape: `{ events: Array<{type, url, network?, creator?}>, capacity: number }`. Capacity caps the event count so the render is bounded. SW writes after every `processUrl` return; gated on `attributionLedgerEnabled`. Pure presenter lives in `src/lib/attribution-ledger.js`; view layer in `src/lib/attribution-ledger-view.js`. |
