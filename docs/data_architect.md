...
| `useNativeShortenerResolution` | boolean | `false` | (ADR-0004 phase 3, #699) Dual-path selector: when ON, the service worker's `UNWRAP_VIA_PROXY` handler resolves shorteners via the native in-browser resolver; when OFF, via the `unwrap.muga.app` proxy. Default OFF. Exposed in advanced settings (dev-mode gated) for first-wave testing. |
