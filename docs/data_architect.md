...
| `cookieConsentMode` | `"off" \| "reject-only"` | `"reject-only"` | Cookie Consent Minimizer mode: `"off"` never opens the gate; `"reject-only"` opens it and only ever rejects / picks an existing necessary-only path via `src/lib/cmp-adapters.js`'s adapters (`content/cookie-noise-mainworld.js` on Chrome MAIN world, `content/cookie-noise.js` on Firefox). MUGA never ships a mode that accepts cookies on the user's behalf. Default `"reject-only"` is for NEW installs only — existing users are migrated to `"off"` by `migrateCookieConsentMode()` (`src/lib/storage-migrations.js`), never silently upgraded. Disclosed via the consent-version 1.2 bump. |
