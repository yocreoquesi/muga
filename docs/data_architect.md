...
| `followShortenersEnabled` | boolean | browser-dependent (`true` on Chrome MV3, `false` on Firefox MV2) | (ADR-0004 phases 2-5, #699/#701) Opt-in on Firefox, on-by-default on Chrome: when ON, MUGA resolves generic shorteners (bit.ly, tinyurl.com, etc.) in-browser via `fetch(redirect:"manual")`. No MUGA server is involved. Requires the shortener host permissions, granted from the options toggle. Chrome MV3 already holds `<all_urls>` as a mandatory host permission, so the fetch works with no extra prompt — the default is `true` there; Firefox MV2 requires an explicit `chrome.permissions.request()` gesture, so the default stays `false`. An explicitly stored value always wins over the computed default (`src/lib/prefs.js` `defaultFollowShortenersEnabled()`). Renamed from `privacyProxyEnabled` in phase 5 (ADR-0004, 2026-06-01); a one-time startup migration copies `privacyProxyEnabled=true` → `followShortenersEnabled=true` and deletes the old key. |
| `hoverPreviewEnabled` | boolean | `true` | (#1028) Desktop-only: `src/content/hover-preview.js` shows a small text-only tooltip with the real cleaned destination when the user hovers AND holds still over a link for `hoverPreviewDelayMs`. Local-only (no network, no new permissions) when local unwrap/clean changes the link's host (wrappers / redirect networks). For a generic shortener link whose host does NOT change locally, it ALSO resolves the destination over the network via the existing `RESOLVE_SHORTENER` round trip — but only when `followShortenersEnabled` is ON, reusing that opt-in rather than requesting a new permission. Default ON. |
...
| `cookieConsentMode` | `"off" \| "reject-only" \| "accept-when-necessary"` | `"reject-only"` | (3-state redesign, supersedes #1027's boolean) Cookie Consent Minimizer mode: `"off"` never opens the gate; `"reject-only"` opens it and only ever rejects / picks an existing necessary-only path via `src/lib/cmp-adapters.js`'s adapters (`content/cookie-noise-mainworld.js` on Chrome MAIN world, `content/cookie-noise.js` on Firefox); `"accept-when-necessary"` is a recognised enum value with no UI or behaviour yet (Slice 2). Default `"reject-only"` is for NEW installs only — existing users are migrated to `"off"` by `migrateCookieConsentMode()` (`src/lib/storage-migrations.js`), never silently upgraded. Disclosed via the consent-version 1.2 (original) and 1.3 (mode-aware) bumps. |
| `cookieConsentAcceptConsented` | boolean | `false` | Hard gate for `cookieConsentMode: "accept-when-necessary"` (Slice 2). Unused in Slice 1 — no code path sets it — but defined ahead of time so the Slice 2 double-gate (mode + this flag) and the settings export/import round-trip are both already in place. Only ever set via an explicit in-UI consent gesture. |
