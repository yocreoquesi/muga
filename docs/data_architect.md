...
| `cookieConsentMinimizerEnabled` | boolean | `false` | (#1027) Opt-in Cookie Consent Minimizer: when a supported CMP (currently OneTrust only, `src/lib/cmp-adapters.js`) exposes a confirmed reject/necessary-only path, MUGA exercises it via `content/cookie-noise-mainworld.js` (Chrome MAIN world) or `content/cookie-noise.js` (Firefox, `window.wrappedJSObject`). On a hard wall (no reject path, only a broad consent-granting action) MUGA does nothing. Default OFF; disclosed via the consent-version 1.2 bump. |
