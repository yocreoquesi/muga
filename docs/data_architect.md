...
| `privacyProxyEnabled` | boolean | `false` | (#453, B20) Opt-in: when ON, opaque affiliate-network redirect URLs that cannot be unwrapped client-side are sent to `unwrap.muga.app` (a Cloudflare Worker) for resolution. Every response is verified with an Ed25519 signature before navigation. Default OFF: requires explicit user consent because it involves a network request with the full affiliate URL. |
| `followShortenersEnabled` | boolean | `false` | (ADR-0004 phase 2, #699) Opt-in: when ON, MUGA resolves the generic shorteners (bit.ly, tinyurl.com, t.co, link.medium.com, lnkd.in, fb.me, ebay.to) in-browser via `fetch(redirect:"manual")` instead of the proxy. Requires the shortener host permissions, granted from the options toggle. Default OFF. (amzn.to is excluded — it sits in opaque-networks' PENDING_VERDICT bucket awaiting #665.) |
| `useNativeShortenerResolution` | boolean | `true` | (ADR-0004 phases 3-4, #699/#700) Dual-path selector: when ON, the service worker's `UNWRAP_VIA_PROXY` handler resolves shorteners via the native in-browser resolver; when OFF, via the `unwrap.muga.app` proxy. Default TRUE as of 2.2.0-beta.1 (phase 4 flipped from false). The proxy remains as fallback when native fails. Exposed in advanced settings (dev-mode gated). |
