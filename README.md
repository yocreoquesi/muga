...
MUGA preserves creator affiliate tags on **6 programs**: Amazon, eBay, Vercel, DigitalOcean, Lemon Squeezy, Apple Performance Partners. The full allowlist lives in [`src/rules/caps-manifest.json`](src/rules/caps-manifest.json); the decision algorithm that governs preserve-vs-strip is documented in [`docs/rules/decision-algorithm.md`](docs/rules/decision-algorithm.md).
