...
- **Operating CAPS as a multi-party standard.** MUGA's decision algorithm and rule artefacts ([`docs/rules/decision-algorithm.md`](docs/rules/decision-algorithm.md), [`src/rules/caps-manifest.json`](src/rules/caps-manifest.json), [`src/rules/caps-wrappers.json`](src/rules/caps-wrappers.json)) are published and verifiable, but governance is single-editor and there is one implementation (MUGA itself). We will not invest in standards-body ceremony — RFCs, conformance certifications, multi-party governance — unless an external implementer adopts the rules independently. If that happens, the rules will be extracted into a separately governed spec at that point. Until then, "the rules" are internal documentation that other tools are welcome to read.
