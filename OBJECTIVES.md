...
- **Operating an external multi-party standard.** MUGA's URL rules (what it preserves, what it strips, and under what conditions) are maintained internally as documented, verifiable artifacts in `src/rules/` and `docs/rules/decision-algorithm.md`, enforced by a CI gate. MUGA does not operate these rules as a separately-governed, multi-party external standard. If an independent implementer were to adopt the same rule shapes, that could motivate extracting the definitions into a separately-governed spec — but that is only worth doing if an external implementer actually exists. Proposing a spec without an independent implementer is premature standardization.
