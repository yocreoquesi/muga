...
**Monthly removal-direction report (#1228):** `anchored-only-globals.yml` runs 1st of the month 08:00 UTC + `workflow_dispatch` (2 h after `moat-expansion.yml`, itself 2 h after `import-upstream.yml`, so the three monthly jobs do not collide). It is the mirror-image measurement to this section's addition-direction pipeline: `tools/anchored-only-globals.mjs` reuses `parseRemoveparamRules` (AdGuard) and a scope-split reader in the ClearURLs adapter to find current `TRACKING_PARAMS` entries with evidence that is ONLY ever host- or path-anchored upstream, never global — the ongoing class first identified by issue #1228 and previously re-derived by hand every few weeks (#1322/#1323/#1324/#1338). It opens or updates one deduplicated issue (fixed title) when there are candidates, and closes it when there are none, exactly like `alert-on-failure.yml`'s dedupe/auto-close pattern. `GITHUB_TOKEN` only (`issues: write`, `contents: read`) — it never edits `TRACKING_PARAMS` itself; every candidate still needs a human to check per-host DNR coverage, guard membership, and self-hosted/SaaS-platform risk.
...
│   ├── domain-rules.json      Per-domain preserve/strip rules (297 entries)
...
tests/
├── unit/                      Node.js test runner — ~4,399+ unit tests
