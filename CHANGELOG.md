...
- **Turning off a tracking category now works on the pages you visit** (#1256). On Chrome, Settings said "Disabling a category keeps those parameters in URLs" and the network layer removed them anyway: those rules are built when the extension is packaged, from the full parameter list, and had no way to know what you had switched off. They are now rebuilt at runtime without the categories you disabled. Firefox was never affected. Amazon's internal-navigation parameters are site rules rather than a tracking category, so they are cleaned either way.
- **Link cleaning in the page was running on a reduced parameter list** (#1255). MUGA rewrites the links on a page as well as the one you click, and that rewriter was meant to use the full cleaning engine, falling back to a small built-in list only in the instant before the engine loads. It was calling the engine incorrectly, so the call failed every time, the failure was swallowed, and the small list was all it ever used. Links in the page are now cleaned with the same rules as the ones you click. The hover tooltip had the same problem in a milder form: it ignored every per-site rule, so it could name a destination the extension would not actually produce.
- **The popup preview and the Settings URL tester now show the real result** (#1255). Both computed their preview without the rules that shorten a path, so an Amazon link still showed its long SEO slug even though the extension removes it. The Settings tester was also missing the referrer, which is what decides whether a creator's referral is kept. Both now clean a URL exactly the way the extension does when you actually open it.
...
- **The privacy policy now states the short-link feature the way it ships** (#1254). Two public documents said "Follow shortener redirects" was off by default. Resolving a link you open has shipped on by default since the feature was split in two, and the policy named 7 of the 19 shortener hosts MUGA can contact. Both policies now list all 19, state each half's real default, and disclose that the "Include the full URL" box on a problem report puts the full address into the issue you submit. A test ties those claims to the code, so the list and the defaults cannot drift again.
- **The privacy policy no longer promises a Terms re-prompt that was removed** (#1254). The policy shipped inside the extension still said MUGA would surface a re-acceptance flow on each device when the Terms changed, and two FAQ pages said the same, one of them citing the public policy as its evidence while that policy said the opposite. [ADR-0007](docs/adr/0007-terms-available-not-accepted.md) removed that engine in 3.0.0. All three now describe what actually happens: the Terms stay reachable, updating them re-prompts nobody and gates nothing.
