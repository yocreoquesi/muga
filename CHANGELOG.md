...
- **Bookshop.org path-based creator-referral exception** (#603, caps-spec#46 deferred). Bookshop's affiliate attribution lives in the path (`/a/{id}/`) and sets a session cookie at entry — there is no `?aff=` query param ever. The cleaner now detects the entry path on `bookshop.org` (and `www.bookshop.org`), preserves it intact, and surfaces a top-level `creatorReferralPreserved: boolean` on the `processUrl` result. The service worker ORs that flag with the existing `preservedAffiliate` check so the standard "Creator referral preserved" toolbar wedge cue fires for Bookshop entry navigations. Out-of-band escape hatch authorised by caps-spec#46 itself; the implementation is deliberately narrow (single-host, single-pattern) so caps-spec stays uninflated until a second path-based program lands.
