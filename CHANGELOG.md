...
### Changed
- Remote rule updates: README and comparison-page descriptions reframed. The previous wording ("Zero outbound requests on a default install") read as a value statement; the actual reason the feature is off by default is interim engineering — the signing infrastructure is still stabilizing. The wording now reflects that, and explicitly notes that the default **may flip in a future release** so the eventual change is predictable rather than a surprise. No code or behavioral change in this entry; existing installs are unaffected. (#362)
- Comparison page (`docs/comparison.html`) refreshed to lead with the wedge. New table column for **Brave (built-in)** added (the strategic review flagged Brave as a missing competitor in the prior matrix). New rows surfacing v1.11.0's user-visible features: "Surfaces preserved creator tag in UI" and "Tracker count celebration in popup" — both rows are MUGA-only, with a footnote pointing to the v1.11.0 release. Existing "Replaces creator affiliates" row updated to honestly reflect that ClearURLs / Brave / Neat URL all *can* strip a creator's tag depending on rule set / filter list version, while MUGA never does. New "Refuses redirect-based affiliate networks" row documents the wedge at the policy level. New plain-language "the wedge in plain English" section above the honest notes. Six new footnotes (16-21) sourcing the new claims. Page now linked from README, the docs landing, and the store listing footer. (#329)
- Store listing rewritten to lead with the differentiator: "Privacy Without Breaking Creator Links". The "fair to creators" wedge — the one MUGA-only behavior that no other URL cleaner can copy without contradicting their own pitch — is now the headline of both Chrome Web Store and Firefox AMO listings, plus README.md and the manifest descriptions. The listing example now shows a third-party reviewer's `?tag=reviewer-21` being preserved while 7 tracking params are removed, so the value proposition is visible in the first 5 seconds of reading. Section ordering reworked: wedge → before/after → param coverage → why we rejected 10+ networks → privacy. New CWS short description (118 chars): "Strip tracking. Keep creator referrals. 450+ params removed automatically. Open source, MV3 native, zero data sent." Strategic review consensus, 2026-04-26 (internal + ChatGPT + Gemini). (#328)
