...
MUGA ships UI in English, Spanish, Portuguese, German, French, Italian, and Japanese. The maintainer is a native Spanish speaker (English fluent); every other locale is best-effort. The project's policy is:
...
- **AI-assisted**: `pt`, `de`, `fr`, `it`, `ja`. Translations are produced with AI assistance and shipped without native-speaker sign-off. The runtime fallback chain at `i18n.js` lands missing entries on EN cleanly. PRs that improve these locales do not require maintainer approval beyond the standard review — the maintainer is not a native speaker of any of these.

To find which keys are missing in any AI-assisted locale, run:
