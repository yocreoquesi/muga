 ...
 *
 * The popup opens on three tiles reading 0, 0, 0 for a fresh install, with no
 * copy anywhere near them. Three bare zeros do not read as "nothing has
 * happened yet", they read as "this is not working" — and the ledger directly
 * below already gets an empty state that says so in words.
 *
...
/** The three counters the popup renders, in the shape storage.local holds. */
const COUNTERS = ["urlsCleaned", "junkRemoved", "referralsSpotted"];
 ...
 *
 * @param {{urlsCleaned?: number, junkRemoved?: number, referralsSpotted?: number}|null|undefined} stats
 * @returns {boolean}
 */
