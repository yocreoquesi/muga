...
export const TRANSLATIONS = {
  ...
  ob_affiliate_desc:        { en: '<strong>Creators come first.</strong> If a creator\'s affiliate tag is already on a link, MUGA never touches it -- we don\'t take credit from people who earned it.<br><br>On selected stores where a link has <strong>no affiliate tag at all</strong>, MUGA can add ours. <strong>Your price never changes.</strong> The code is open source, you can verify it.<br><br>We deliberately rejected 10+ stores whose attribution methods require routing your clicks through external servers. We would rather earn less than compromise how MUGA works.', es: '<strong>Los creadores son lo primero.</strong> Si el tag de afiliado de un creador ya está en un enlace, MUGA nunca lo toca -- no tomamos crédito de quienes se lo ganaron.<br><br>En tiendas seleccionadas donde un enlace <strong>no tiene ningún tag de afiliado</strong>, MUGA puede añadir el nuestro. <strong>Tu precio nunca cambia.</strong> El código es open source, puedes comprobarlo.<br><br>Rechazamos deliberadamente más de 10 tiendas cuyos métodos de atribución requieren enrutar tus clics por servidores externos. Preferimos ganar menos que comprometer cómo funciona MUGA.', pt: '<strong>Os criadores vêm primeiro.</strong> Se o tag de afiliado de um criador já está num link, MUGA nunca o toca -- não tomamos crédito de quem o ganhou.<br><br>Em lojas selecionadas onde um link <strong>não tem nenhum tag de afiliado</strong>, MUGA pode adicionar o nosso. <strong>Seu preço nunca muda.</strong> O código é open source, você pode verificar.<br><br>Rejeitamos deliberadamente mais de 10 lojas cujos métodos de atribuição exigem rotear seus cliques por servidores externos. Preferimos ganhar menos a comprometer como o MUGA funciona.', de: '<strong>Creator haben Vorrang.</strong> Wenn das Affiliate-Tag eines Creators bereits in einem Link vorhanden ist, berühren wir es nie -- wir nehmen kein Guthaben von denen, die es verdient haben.<br><br>In ausgewählten Shops, wo ein Link <strong>überhaupt kein Affiliate-Tag hat</strong>, kann MUGA unseres hinzufügen. <strong>Dein Preis ändert sich nie.</strong> Der Code ist open source, du kannst das überprüfen.<br><br>Wir haben bewusst 10+ Shops abgelehnt, deren Attributionsmethoden erfordern, deine Klicks über externe Server zu leiten. Wir verdienen lieber weniger, als zu gefährden, wie MUGA funktioniert.', fr: "<strong>Les créateurs passent en premier.</strong> Si le tag d'affiliation d'un créateur est déjà sur un lien, MUGA n'y touche jamais -- nous ne prenons pas le crédit des gens qui l'ont mérité.<br><br>Sur certaines boutiques où un lien n'a <strong>aucun tag d'affiliation</strong>, MUGA peut ajouter le nôtre. <strong>Votre prix ne change jamais.</strong> Le code est open source, vous pouvez le vérifier.<br><br>Nous avons délibérément refusé plus de 10 boutiques dont les méthodes d'attribution nécessitent d'acheminer vos clics via des serveurs externes. Nous préférons gagner moins plutôt que compromettre le fonctionnement de MUGA.", it: "<strong>I creator vengono prima.</strong> Se il tag di affiliazione di un creator è già su un link, MUGA non lo tocca mai -- non prendiamo credito da chi se lo è guadagnato.<br><br>Su negozi selezionati dove un link <strong>non ha alcun tag di affiliazione</strong>, MUGA può aggiungere il nostro. <strong>Il tuo prezzo non cambia mai.</strong> Il codice è open source, puoi verificarlo.<br><br>Abbiamo deliberatamente rifiutato oltre 10 negozi i cui metodi di attribuzione richiedono di instradare i tuoi clic attraverso server esterni. Preferiamo guadagnare meno piuttosto che compromettere il funzionamento di MUGA.", ja: "<strong>クリエイターが最優先です。</strong>クリエイターのアフィリエイトタグがすでにリンクにある場合、MUGAは一切触れません -- 価値を生んだ人からクレジットを奪いません。<br><br>アフィリエイトタグが<strong>まったくない</strong>リンクの対象ストアでは、MUGAが当方のタグを追加できます。<strong>価格は変わりません。</strong>コードはオープンソースで検証可能です。<br><br>クリックを外部サーバー経由でルーティングする帰属方法を要求する10以上のストアを意図的に拒否しました。MUGAの動作を損ねるくらいなら収益を減らすほうがいいと考えています。" },
