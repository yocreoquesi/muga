...
// Note: a #1448-specific withScopedRemoteParams helper used to live here,
// merging host-scoped remote facts into the Firefox listener's effective
// remoteParams. Superseded by #1409 (fix(cleaner): apply host-scoped remote
// facts on processUrl paths): computeNavigationStrip -> processUrl now folds
// prefs.remoteScopedFacts in for every caller via cleaner.js/scoped-params.js,
// so no Firefox-specific merge step is needed — see
// tests/unit/scoped-facts-process-url.test.mjs for that coverage.

// ── #1448 — reconcileRemoteDnrRule never installs a remote-channel DNR rule on Firefox ──

describe("#1448 — reconcileRemoteDnrRule on Firefox: remove-only, never adds", () => {
  ...
  test("isFirefoxMV2() reports true under this stub (sanity)", () => {
    installFirefoxChromeStub();
    assert.strictEqual(dnrSync.isFirefoxMV2(), true);
  });

  test("exactly one remove-only call, even with remoteRulesEnabled:true", async () => {
    installFirefoxChromeStub();
    await dnrSync.reconcileRemoteDnrRule({ remoteRulesEnabled: true });
    ...
    assert.ok(
      fakeDnr.calls[0].removeRuleIds.includes(DNR_IDS.DNR_REMOTE_PARAMS_RULE_ID),
      "must remove the global remote rule (1001)",
    );
  });

  test("clears the FULL host-scoped range (3100-5099), not just its base id", async () => {
    installFirefoxChromeStub();
    const { SCOPED_RULE_ID_RANGE } = await import("../../src/lib/remote-rules.js");
    await dnrSync.reconcileRemoteDnrRule({ remoteRulesEnabled: true });
    for (const id of SCOPED_RULE_ID_RANGE) {
      assert.ok(
        fakeDnr.calls[0].removeRuleIds.includes(id),
        `expected scoped-range id ${id} in removeRuleIds`,
      );
    }
  });

  test("never touches chrome.storage — returns before reading the cache", async () => {
    installFirefoxChromeStub();
    // No chrome.storage is defined by this stub at all; reaching the
    // getRemoteParams() call below the Firefox short-circuit would throw.
