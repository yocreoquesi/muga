...
const BASELINE = {
  ...
  "service-worker-patterns.test.mjs": 80,   // SW not importable; behavioral migration is long arc (#824). +1 for the FORCE_FETCH_REMOTE_RULES "Update now" handler existence guard (Update now feature) — the (a)/(b)/(c) gate coverage itself is behavioral via a pure forceFetchRemoteRules() mirror, only the "handler exists" check is source-text. +2 for cookie-consent-accept Slice 2a's modeActive gate-wiring guard (import + computed field): the service worker isn't importable in Node (top-level chrome.* globals), so this stays a source-text assertion until a future SW behavioral-harness migration. +1 for #1027 Slice 2 PR B1's maybeFetchRemoteTier2Rules wake-schedule wiring guard — the gate logic itself (remoteRulesEnabled/consent/freshness) is covered behaviorally via a pure makeMaybeFetchTier2Helper() mirror, only the "wired onto the schedule" check is source-text.
