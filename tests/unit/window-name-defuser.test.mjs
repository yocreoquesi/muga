...
describe("window-name-defuser — content-script wiring", () => {
  ...
  test("manifest.v2.json registers the isolated gate but NOT the mainworld wrap (#1026)", () => {
    // window-name-defuser-mainworld.js is Chrome-MV3-only (world:MAIN). On
    // Firefox MV2 the page-world wrap is done directly by
    // window-name-defuser.js via window.wrappedJSObject + exportFunction, so
    // the mainworld file must not be loaded at all on MV2.
    const manifest = JSON.parse(readFileSync(
    ...
    assert.equal(wrapEntry, undefined,
      "window-name-defuser-mainworld.js must NOT be registered for MV2 (Chrome-MV3-only)");
  });
