...
// ── #1027 Slice 2 PR B1: Tier2 sibling of the on-wake fetch ─────────────────
// maybeFetchRemoteTier2Rules rides the EXACT SAME wake-based schedule and
// REMOTE_REFRESH_INTERVAL_MS cadence as maybeFetchRemoteRules above, gated by
// the SAME remoteRulesEnabled + consent state (spec req 5 — no new toggle),
// but tracks its own remoteTier2Meta.fetchedAt freshness key so a Tier2
// failure is an isolated failure domain (design ADR-9). Behavioral coverage
// mirrors the makeMaybeFetchHelper() idiom above (#824 — prefer a pure
// re-implementation exercised behaviorally over source-text assertions).
// Only ONE minimal source guard is added below to pin that the real wiring
// exists in the SW (baseline bumped by exactly 1 in source-grep-ratchet.test.mjs).

/**
 * Pure extraction of maybeFetchRemoteTier2Rules for unit testing. Mirrors
 * the production logic in service-worker.js exactly: same gate, same order.
 */
function makeMaybeFetchTier2Helper() {
  let _checked = false;
  return async function maybeFetchRemoteTier2Rules(deps) {
    if (_checked) return "skipped-dedup";
    _checked = true;
    const prefs = await deps.getPrefs();
    if (!prefs.remoteRulesEnabled) return "disabled";
    if (evaluateConsent({ stored: prefs }).status !== "valid") return "consent-blocked";
    const { remoteTier2Meta } = await deps.getTier2Meta();
...
describe("Tier2 remote-rules on-wake time-gated fetch (PR B1, mirrors params)", () => {
  test("short-circuits when remoteRulesEnabled is false", async () => {
    let fetchCalled = false;
    const maybe = makeMaybeFetchTier2Helper();
    const result = await maybe({
      getPrefs: async () => ({ remoteRulesEnabled: false }),
      getTier2Meta: async () => ({ remoteTier2Meta: { fetchedAt: null } }),
      runFetch: async () => { fetchCalled = true; },
      fetchDeps: {},
    });
    assert.strictEqual(result, "disabled");
  ...
  test("does NOT fetch when consent is not yet valid (rides the SAME gate as params)", async () => {
    let fetchCalled = false;
    const maybe = makeMaybeFetchTier2Helper();
    const result = await maybe({
      getPrefs: async () => ({ remoteRulesEnabled: true }), // no onboardingDone
      getTier2Meta: async () => ({ remoteTier2Meta: { fetchedAt: null } }),
      runFetch: async () => { fetchCalled = true; },
      fetchDeps: {},
    });
    assert.strictEqual(result, "consent-blocked");
    assert.strictEqual(fetchCalled, false, "no new toggle — Tier2 fetch must not bypass the params consent gate");
  });

  test("short-circuits when last Tier2 fetch is fresh (< 7 days)", async () => {
    let fetchCalled = false;
    const maybe = makeMaybeFetchTier2Helper();
    const recent = new Date(Date.now() - 60 * 60 * 1000).toISOString();
    const result = await maybe({
      getPrefs: async () => ({ remoteRulesEnabled: true, ...VALID_CONSENT }),
      getTier2Meta: async () => ({ remoteTier2Meta: { fetchedAt: recent } }),
      runFetch: async () => { fetchCalled = true; },
      fetchDeps: {},
    });
    assert.strictEqual(result, "fresh");
  ...
  test("fires fetch when last Tier2 fetch is stale (> 7 days)", async () => {
    let fetchCalled = false;
    const maybe = makeMaybeFetchTier2Helper();
    const stale = new Date(Date.now() - 8 * 24 * 60 * 60 * 1000).toISOString();
    const result = await maybe({
      getPrefs: async () => ({ remoteRulesEnabled: true, ...VALID_CONSENT }),
      getTier2Meta: async () => ({ remoteTier2Meta: { fetchedAt: stale } }),
      runFetch: async () => { fetchCalled = true; },
      fetchDeps: {},
    });
    assert.strictEqual(result, "ran");
  ...
  test("fires fetch when remoteTier2Meta is absent (first-time enable)", async () => {
    let fetchCalled = false;
    const maybe = makeMaybeFetchTier2Helper();
    const result = await maybe({
      getPrefs: async () => ({ remoteRulesEnabled: true, ...VALID_CONSENT }),
      getTier2Meta: async () => ({ remoteTier2Meta: null }),
      runFetch: async () => { fetchCalled = true; },
      fetchDeps: {},
    });
    assert.strictEqual(result, "ran");
  ...
  test("dedupes subsequent calls in the same SW lifetime (own flag, isolated from params)", async () => {
    let fetchCount = 0;
    const maybe = makeMaybeFetchTier2Helper();
    const deps = {
      getPrefs: async () => ({ remoteRulesEnabled: true, ...VALID_CONSENT }),
  ...
  test("SW wires runTier2RulesFetch into the on-wake schedule", () => {
    // Single minimal source guard (#824 — only one new swSource.* assertion
    // added for this feature; the gate logic itself is covered behaviorally
    // above via the pure re-implementation).
    assert.ok(
      swSource.includes("maybeFetchRemoteTier2Rules(_remoteTier2RulesDeps())"),
      "SW must wire the Tier2 sibling fetch onto the same wake-based schedule as params"
    );
  });
});
