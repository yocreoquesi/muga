...
// ── Cookie-gate channel: manifest ordering pin (#1027) ────────────────────────
//
// The Cookie Consent Minimizer uses a SEPARATE nonce handshake on the
// muga:cookie-gate channel. The isolated-world gatekeeper
// (content/cookie-noise.js) generates the nonce, fires the one-shot
// muga:cookie-gate:nonce event, then dispatches muga:cookie-gate. The
// MAIN-world caller (content/cookie-noise-mainworld.js) must have registered
// its nonce listener BEFORE that one-shot fires. In Chrome MV3, world:"MAIN"
// scripts run as a phase before any isolated group, so this holds — but a
// manifest reorder must not be able to break it silently. Mirrors the
// history-gate pin above, scoped to the cookie-gate dispatcher.
//
// Firefox MV2 has no world:"MAIN" and does NOT load the MAIN-world caller at
// all (it uses the direct wrappedJSObject reject path), so there is no
// cross-world handshake and no ordering constraint there.

describe("gate-nonce — cookie-gate channel: dispatcher runs after its listener", () => {
  const COOKIE_DISPATCHER = "content/cookie-noise.js";
  const COOKIE_NONCE_PATTERN = /muga:cookie-gate:nonce/;
  ...
  function flattenExecutionOrder(manifestPath) {
    ...
    for (const group of groups) {
      if (!Array.isArray(group.js)) continue;
      const world = group.world || "ISOLATED";
      if (world === "MAIN") mainScripts.push(...group.js);
      else otherScripts.push(...group.js);
    }
    return [...mainScripts, ...otherScripts];
  }

  function findCookieNonceRegistrants(scripts) {
    const srcRoot = join(__dirname, "../../src");
    const registrants = [];
    for (const scriptPath of scripts) {
      if (scriptPath === COOKIE_DISPATCHER) continue; // skip dispatcher itself
      let source;
      try {
        source = readFileSync(join(srcRoot, scriptPath), "utf8");
      } catch {
      ...
      if (COOKIE_NONCE_PATTERN.test(source)) registrants.push(scriptPath);
    }
    return registrants;
  }

  test("src/manifest.json (MV3): cookie-noise-mainworld.js registers before content/cookie-noise.js", () => {
    const scripts = flattenExecutionOrder(join(__dirname, "../../src/manifest.json"));
    const dispatcherIdx = scripts.indexOf(COOKIE_DISPATCHER);
    assert.ok(dispatcherIdx !== -1, "cookie-gate dispatcher not found in MV3 execution order");

    const registrants = findCookieNonceRegistrants(scripts);
    assert.ok(
      registrants.includes("content/cookie-noise-mainworld.js"),
      "MV3 must load the MAIN-world cookie-gate listener as a nonce registrant",
    );

    for (const registrant of registrants) {
      const idx = scripts.indexOf(registrant);
      assert.ok(
        idx < dispatcherIdx,
        `${registrant} (index ${idx}) must come BEFORE ${COOKIE_DISPATCHER} (index ${dispatcherIdx}) — the listener must register before the dispatcher fires the one-shot nonce`,
      );
    }
  });

  test("src/manifest.v2.json (Firefox MV2): no cross-world cookie-gate handshake (direct reject path)", () => {
    const scripts = flattenExecutionOrder(join(__dirname, "../../src/manifest.v2.json"));
    const registrants = findCookieNonceRegistrants(scripts);
    assert.deepEqual(
      registrants,
      [],
      "Firefox MV2 must not carry a MAIN-world cookie-gate listener — it uses the direct wrappedJSObject reject path",
    );
  });
});
